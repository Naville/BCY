...
bool DownloadFilter::shouldBlockDetail(web::json::value abstract,string item_id) {
  ...
  if (ScriptList.size() > 0) {
    ...
    for (web::json::value j : ScriptList) {
      ...
      try {
        ...
        if (result > 0) {
          BOOST_LOG_TRIVIAL(debug) << item_id << " Allowed by Script" << suff << endl;
          return result;
        } else if (result < 0) {
          BOOST_LOG_TRIVIAL(debug) << item_id << " Blocked by Script" << suff << endl;
          return result;
        } else {
          continue;
        }
        ...
        BOOST_LOG_TRIVIAL(error)
            << "During Evalution Script:"<<endl << scpt <<endl<< "on item_id:" << item_id
...
bool DownloadFilter::shouldBlockAbstract(web::json::value abstract,string item_id) {
