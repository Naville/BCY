...
if __name__ == '__main__':
    ...
    newDB.execute("CREATE TABLE IF NOT EXISTS ItemInfo (uid INTEGER DEFAULT 0,item_id INTEGER DEFAULT 0,Title STRING NOT NULL DEFAULT '',Tags STRING NOT NULL DEFAULT '[]',ctime INTEGER DEFAULT 0,Description STRING NOT NULL DEFAULT '',Images STRING NOT NULL DEFAULT '[]',UNIQUE (item_id) ON CONFLICT REPLACE)")
    ...
    for item in x:
        ...
        Tags=item[3]
        Title=item[4]
        newDB.execute("INSERT INTO ItemInfo(uid,item_id,Tags,ctime,Description,Images,Title) VALUES(?,?,?,?,?,?,?)",(uid,item_id,Tags,ctime,plain,Multi,Title,))
    mapping.clear()
    oldDB.commit()
    oldDB.close()
    newDB.commit()
